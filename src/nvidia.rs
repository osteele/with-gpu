...
pub fn query_gpus() -> Result<Vec<GpuInfo>> {
    let nvml = Nvml::init().context("Failed to initialize NVML (is the NVIDIA driver installed?)")?;

    let device_count = nvml.device_count().context("Failed to get GPU count")?;

    let mut gpus = Vec::new();
    for i in 0..device_count {
        let device = nvml.device_by_index(i).context(format!("Failed to get GPU {}", i))?;
